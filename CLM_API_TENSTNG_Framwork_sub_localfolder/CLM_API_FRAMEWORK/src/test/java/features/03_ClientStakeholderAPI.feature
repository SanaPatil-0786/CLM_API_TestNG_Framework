...
Given a prequest payload is prepared for the "SaveClientInformation" API
...
Given a prequest payload is prepared for the "SaveClientAddress" API
...
Given a prequest payload is prepared for the "SaveClientContactInformation" API
...
Given a prequest payload is prepared for the "DeleteClientAddress" API 
...
Given a prequest payload is prepared for the "DeleteClientContactInformation" API 
...
Given a prequest payload is prepared for the "GetAllClientList" API
...
Given a prequest payload is prepared for the "GetClientContactWithAddressList" API
When a GET request is send to "GetClientContactWithAddressList" API
Then the response status code should "200"
And check all contacts and address list is available in response body

@run
Scenario: verify searched contact person details are correct display
Given a prequest payload is prepared for the "GetKeyStakeHolderInfo" API
...
Given a prequest payload is prepared for the "SaveStakeHoldersClientDetails" API
